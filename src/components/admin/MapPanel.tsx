  ...
  return (
    <div className="relative rounded-2xl glass gradient-border overflow-hidden h-[460px]">
      ...
      <div className="absolute top-4 left-4 right-4 flex items-start justify-between pointer-events-none">
        ...
        <div className="flex items-center gap-2 pointer-events-auto">
          {(["worker","job","business","emergency"] as const).map((k) => (
            <span key={k} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg glass text-[10px] text-white/70 capitalize">
      ...
      <div className="absolute right-4 bottom-4 flex flex-col rounded-xl glass overflow-hidden">
        {[Plus, Minus, Crosshair, Layers, Maximize2].map((Ic, i) => (
          <button key={i} className="grid place-items-center h-9 w-9 text-white/70 hover:text-white hover:bg-white/5 border-b border-white/5 last:border-0">
